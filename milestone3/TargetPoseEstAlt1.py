...
def estimate_pose(base_dir, camera_matrix, completed_img_dict):
    ...
    # redapple_dimensions = [0.074, 0.074, 0.087]
    # target_dimensions.append(redapple_dimensions)
    # greenapple_dimensions = [0.0803, 0.0791, 0.0685]
    # target_dimensions.append(greenapple_dimensions)
    # orange_dimensions = [0.075, 0.075, 0.071]
    # target_dimensions.append(orange_dimensions)
    # mango_dimensions = [0.113, 0.067, 0.058] # measurements when laying down
    # target_dimensions.append(mango_dimensions)
    # capsicum_dimensions = [0.0706, 0.0666, 0.099]
    # target_dimensions.append(capsicum_dimensions)
    ...
    for target_num in completed_img_dict.keys():
        ...
        X_C = camera_matrix[0][2] #x-offset to the optical centre
        d = focal_length * true_height / box[3][0] #focal/d = h/true_h
        X = (box[0][0] - X_C) * true_height / box[3][0] #X/x = true_h/h
        print('target num: ', target_num)
        print('focal length: ', focal_length)
        print('true_height: ', true_height)
        print('image height: ', box[3][0])
        print('distance: ', d)
        print('X: ', X)
        print('X_C: ', X_C)
        # target_pose['x'] = robot_pose[0][0] + d*np.sin(-robot_pose[2][0]) + X*np.cos(-robot_pose[2][0])
        # target_pose['y'] = robot_pose[1][0] + d*np.cos(-robot_pose[2][0]) - X*np.sin(-robot_pose[2][0])
        target_pose['x'] = robot_pose[0][0] + d*np.cos(robot_pose[2][0]) + X*np.sin(robot_pose[2][0])
        target_pose['y'] = robot_pose[1][0] + d*np.sin(robot_pose[2][0]) - X*np.cos(robot_pose[2][0])
