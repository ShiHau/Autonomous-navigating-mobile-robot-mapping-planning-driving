...
class calibration:
    ...
    def image_collection(self, dataDir, i):
        if self.command['image'] == True:
            image = self.pibot.get_image()
            filename = "Images/Two_Fruits2_{}.png".format(i)
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.imwrite(filename, image)
            print('Sucessfully captured image: Fruit_{}.png'.format(i))
    ...
    def control(self):
        ...
        lv, rv = self.pibot.set_velocity(motion_command,[15,15])
...
if __name__ == "__main__":
    ...
    print('Images stored in folder calibration/Images')
    ...
    while not calib.finish:
        ...
        calib.control()
        calib.take_pic()
        i = calib.image_collection(dataDir, i)
