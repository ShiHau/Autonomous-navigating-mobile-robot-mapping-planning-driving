...
def calib_Rob_Pose():
    ...
    turn_time = full_turn_time  * 45/360
    ...
    for i in range(0,4):
        ...
        img_1 = ppi.get_image()
        time.sleep(1)
        move_2 = ppi.set_velocity([0,1],tick=[10,-10],time=turn_time)
        img_2 = ppi.get_image()
        time.sleep(1)
        robot_pose = get_robot_pose(move_1, move_2, img_1, img_2)
        print("Calibrate robot pose: ", robot_pose, waypoint)
    move_1 = ppi.set_velocity([0,1],tick=[10,-10],time=turn_time)
    img_1 = ppi.get_image()
    move_2 = ppi.set_velocity([0,1],tick=[-10,10],time=full_turn_time)
    img_2 = ppi.get_image()
    time.sleep(1)
    robot_pose = get_robot_pose(move_1, move_2, img_1, img_1)
    print("Calibrate robot pose: ", robot_pose, waypoint)
...
if __name__ == "__main__":
    ...
    grid_size, robot_radius, offset = 0.4, 0.12, 0.20  # [m]
    ...
    for i in range(0,len(fruits_true_pos_seq)):
        ...
        for j in range(len(rx)-1,-1,-1): # move robot to specified waypoint
            waypoint[0][0] = rx[j]
            waypoint[0][1] = ry[j]
            if waypoint[0][0] != 0.0 and waypoint[0][1] != 0.0:
