...
class EKF:
    ...
    def load_map(self,slam_map_name):
        utils = MappingUtils()
        utils.load(slam_map_name)

        self.taglist = utils.taglist
        self.markers = utils.markers

        self.P = np.zeros((23,23))
        self.P[3:, 3:] = utils.covariance
    ...
    # https://atsushisakai.github.io/PythonRobotics/modules/localization/extended_kalman_filter_localization_files/extended_kalman_filter_localization.html
    ...
    def predict_covariance(self, raw_drive_meas):
        n = self.number_landmarks()*2 + 3
        Q = np.zeros((n,n))
        Q[0:3,0:3] = self.robot.covariance_drive(raw_drive_meas)+ 0.025*np.eye(3)
        return Q
