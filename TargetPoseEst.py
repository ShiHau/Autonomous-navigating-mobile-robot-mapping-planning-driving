...
def merge_estimations(target_map):
    ...
    for i in range(num_per_target):
        try:
            target_est['redapple'] = {'x':redapple_est[i][0].tolist(), 'y':redapple_est[i][1].tolist()}
        except:
            pass
        try:
            target_est['greenapple'] = {'x':greenapple_est[i][0].tolist(), 'y':greenapple_est[i][1].tolist()}
        except:
            pass
        try:
            target_est['orange'] = {'x':orange_est[i][0].tolist(), 'y':orange_est[i][1].tolist()}
        except:
            pass
        try:
            target_est['mango'] = {'x':mango_est[i][0].tolist(), 'y':mango_est[i][1].tolist()}
        except:
            pass
        try:
            target_est['capsicum'] = {'x':capsicum_est[i][0].tolist(), 'y':capsicum_est[i][1].tolist()}
        except:
            pass
    ###########################################
        
    return target_est
def get_bounding_box_auto(target_number, image_path):
    # image = PIL.Image.open(image_path).resize((640,480), PIL.Image.Resampling.NEAREST)
    image = image_path
    target = Image(image)==target_number
    blobs = target.blobs()
...
# read in the list of detection results with bounding boxes and their matching robot pose info
# def get_image_info(base_dir, file_path, image_poses):
def get_image_info_auto(image, image_poses):
    ...
    # target labels: 1 = redapple, 2 = greenapple, 3 = orange, 4 = mango, 5=capsicum, 0 = not_a_target
    # img_vals = set(Image(base_dir / file_path, grey=True).image.reshape(-1))
    img_vals = set(list(image.reshape(-1).astype(int)))
    for target_num in img_vals:
        if target_num > 0:
            try:
                # box = get_bounding_box(target_num, base_dir/file_path) # [x,y,width,height]
                box = get_bounding_box_auto(target_num, image) # [x,y,width,height]
                offset_x = math.ceil(box[2]/2)
                offset_y = math.ceil(box[3]/2)
                # pose = image_poses[file_path] # [x, y, theta]
                #### check if the edge of bounding box touches edge of picture ####
                if not(((box[0] + offset_x) >= 639) and ((box[0] - offset_x) <= 0) and ((box[1] + offset_y) >= 319) and ((box[1] - offset_y) <= 0)):
                    pose = image_poses # [x, y, theta]
...
def estimate_pose_auto(camera_matrix, completed_img_dict):
    camera_matrix = camera_matrix
