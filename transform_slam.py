...
def transform_map(theta, x, points):
    ...
    points_transformedy = np.append(points[1][0]+0.1, points_transformedy)
    points_transformedx = points_transformed1[0]
    points_transformedx = np.append(points[0][0]+0.1, points_transformedx)
    points_transformed = [points_transformedx.tolist(), points_transformedy.tolist()]
    return points_transformed

def transform_fruits(theta, x, targets):
    c, s = np.cos(-theta), np.sin(-theta)
    R = np.array(((c, -s), (s, c)))
    targets_transformed =  R @ targets - x
    return targets_transformed.tolist()
...
if __name__ == '__main__':
    ...
    robot_pose_origin[2] = 0.0*np.pi/180 # +cw -acw
    ...
    #fruit_list, fruit_pos = get_targets('lab_output/targets.txt')
    #fruit_pos = np.array(fruit_pos).T
    ...
    #fruits_transformed = transform_fruits(theta, origin, fruit_pos)
    ...
    ax.scatter(fruits_transformed[0],fruits_transformed[1], marker='o', color='C2',s=40)
    for i in range(len(taglist)):
        ax.text(aruco_transformed[0][i]+0.05, aruco_transformed[1][i]+0.05, taglist[i], color='C0', size=8)
    for i in range(len(fruit_list)):
       ax.text(fruits_transformed[0][i]+0.05, fruits_transformed[1][i]+0.05, fruit_list[i], color='C2', size=8)
    ...
    for i in range(0,len(fruit_list)):
        target_pose_dict[fruit_list[i]] = {'x':fruits_transformed[0][i], 'y':fruits_transformed[1][i]}
